defmodule Instagram.Feeds do
  ...
  def add_message(feed_id, message_params) do
    message_params
    |> Map.put("feed_id", feed_id)
    |> Messages.create_message()
  end
  def search(search_params) do
    query = from(f in Feed, where: fragment("? ILIKE ?", f.author, ^"%#{search_params}%") or fragment("? ILIKE ?", f.title, ^"%#{search_params}%"))
